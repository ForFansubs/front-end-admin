...
export default function EpisodeUpdate(props) {
    ...
    async function handleVisibilityButtonClick(episode_id) {
        ...
        if (res.status === 200) {
            ...
            ToastNotification(payload("process-success", "success", res.data.message || "Görünürlük başarıyla değiştirildi."))
        }

        else {
            ToastNotification(payload("process-error", "error", res.response.data.err || "Görünürlük değiştirilirken bir sorunla karşılaştık."))
        }
    }
    ...
    async function handleDataSubmit(th) {
        ...
        if (res.status === 200) {
            ...
            handleClose()
            ToastNotification(payload("process-success", "success", res.data.message || "Bölüm bilgileri başarıyla değiştirildi."))
        }

        else {
            ToastNotification(payload("process-error", "error", res.response.data.err || "Bölüm bilgileri değiştirilirken bir sorunla karşılaştık."))
        }
    }
